...
void remote::setJoystickPins(int x1_pin, int y1_pin, int x2_pin, int y2_name){
    this->x1_pin = x1_pin;
    this->y1_pin = y1_pin;
    this->x2_pin = x2_pin;
    this->y2_pin = y2_pin;
...
void remote::setSerial(HardwareSerial* serial){
    this->serial = serial;
}
...
void remote::encode_joysticks(){
    msg.x = to_percentage(analogRead(x1_pin), x_min, x_mid, x_max);  //reverse
    msg.y = to_percentage(analogRead(y1_pin), y_max, y_mid, y_min);
    msg.w = to_percentage(analogRead(x2_pin), w_min, w_mid, w_max);  //reverse
...
int8_t remote::to_percentage(int value, int max, int mid, int min){
  ...
  return (int8_t)map(value, min, max, -100, 100);
}

void remote::cal_battery_volt(){
  int temp = analogRead(battery_level_pin);
  ...
  volt_percentage = map(volt*1000, volt_min*1000, volt_max*1000, 0.0f, 100.0f*1000)/1000.0f;
}
